...
def main():
    ...
    with tf.Session() as sess:
        ...
        for text in test_data:
            ...
            input_feed = {model.seq_input.name: data, model.target.name: targets, model.seq_lengths.name: seq_lengths}
            output_feed = [model.y]
...
def prepareText(text, max_seq_length, vocab_mapping):
    ...
    if len(indices) < max_seq_length:
        indices += [vocab_mapping.getIndex("<PAD>") for i in range(max_seq_length - len(indices))]
    else:
